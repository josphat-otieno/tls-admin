    ...
    <body class="loading" data-layout-color="light" data-layout-mode="default" data-layout-size="fluid"
        ...
        <script>
                ...
                deliverables.forEach(d => {
                    ...
                    div.innerHTML = `
                        <div class="existing-deliverable-info">
                            ...
                            <br>
                            <small class="text-muted">${d.file_path.startsWith('http') ? d.file_path : d.file_path.split('/').pop()}</small>
                        </div>
                ...
                div.innerHTML = `
                    ...
                    <div class="row">
                        <div class="col-md-12 mb-2">
                            ...
                            <input type="text" name="deliverable_titles[${deliverableIndex}]" class="form-control form-control-sm" placeholder="Title (e.g. Logo Design)">
                        </div>
                        <div class="col-md-6 mb-2">
                            <label class="form-label small">Media Type</label>
                            <select name="deliverable_media_types[${deliverableIndex}]" class="form-select form-select-sm" onchange="toggleVideoInput(this, ${deliverableIndex})">
                        ...
                        <div class="col-md-6 mb-2">
                            ...
                            <select name="deliverable_types[${deliverableIndex}]" class="form-select form-select-sm">
                                ${typesOptions}
                            </select>
                        </div>
                        <div class="col-md-12 mb-2" id="video-options-container-${deliverableIndex}" style="display: none;">
                            <div class="form-check form-check-inline mt-1">
                                <input class="form-check-input" type="radio" name="video_source_${deliverableIndex}" id="video_upload_${deliverableIndex}" value="upload" checked onchange="toggleVideoSource(${deliverableIndex})">
                                <label class="form-check-label small" for="video_upload_${deliverableIndex}">Upload File</label>
                            </div>
                            <div class="form-check form-check-inline mt-1">
                                <input class="form-check-input" type="radio" name="video_source_${deliverableIndex}" id="video_youtube_${deliverableIndex}" value="youtube" onchange="toggleVideoSource(${deliverableIndex})">
                                <label class="form-check-label small" for="video_youtube_${deliverableIndex}">YouTube URL</label>
                            </div>
                        </div>
                        <div class="col-md-12" id="file-input-container-${deliverableIndex}">
                            <label class="form-label small">File</label>
                            <input type="file" name="deliverable_files[${deliverableIndex}]" id="file_input_${deliverableIndex}" class="form-control form-control-sm" required>
                        </div>
                        <div class="col-md-12 mt-2" id="youtube-input-container-${deliverableIndex}" style="display: none;">
                            <label class="form-label small">YouTube URL</label>
                            <input type="url" name="deliverable_youtube_urls[${deliverableIndex}]" id="youtube_input_${deliverableIndex}" class="form-control form-control-sm" placeholder="https://www.youtube.com/watch?v=...">
                        </div>
                    </div>
                `;
                container.appendChild(div);
            ...
            function toggleVideoInput(selectElement, index) {
                const videoOptionsContainer = document.getElementById(`video-options-container-${index}`);
                if (selectElement.value === 'Video') {
                    videoOptionsContainer.style.display = 'block';
                } else {
                    videoOptionsContainer.style.display = 'none';
                    document.getElementById(`video_upload_${index}`).checked = true;
                    toggleVideoSource(index);
                }
            }

            function toggleVideoSource(index) {
                const isYoutube = document.getElementById(`video_youtube_${index}`).checked;
                const fileContainer = document.getElementById(`file-input-container-${index}`);
                const youtubeContainer = document.getElementById(`youtube-input-container-${index}`);
                const fileInput = document.getElementById(`file_input_${index}`);
                const youtubeInput = document.getElementById(`youtube_input_${index}`);
